...
export type CapEvent = { dayOffset: number; label: string; hours: number; project?: string; aiTool: string } // Added aiTool for explicit mention
    ...
    <div className="gantt">
      <div className="ganttGrid" style={{ gridTemplateColumns: gridCols }}>
        ...
        {Array.from({ length: spanDays }).map((_, i) => {
          ...
          return (
            <div key={i} className="dayCell">
              {evs.map((ev, idx) => (
                <div key={idx} className="capacityBlock">
                  +{ev.hours}h from {ev.aiTool} in {ev.project}
                </div>
              ))}
            </div>
          )
        ...
        <div className="actionCell">
          <span className="small">Pills show time freed by AI tools in accelerated projects.</span>
        </div>
