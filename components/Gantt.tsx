...
type Task = {
  ...
  codPerDay: number; mvi: number; status: 'open'|'closed'; allocatedMinutes?: number;
}
...
export default function Gantt({tasks, onAllocate, onComplete}:Props){
  const { start, end, spanDays, gridCols, rows } = useMemo(()=>{
    ...
    return { start, end, spanDays, gridCols, rows }
  },[tasks])
          ...
          return (
            <div key={r.id} className="ganttRow">
              <div className="taskCell">
                <div>{r.name}</div>
                <div className="small">{r.project} • Baseline {r.baselineDays}d • CoD ${r.codPerDay.toLocaleString()}</div>
              </div>
              {Array.from({length: spanDays}).map((_,i)=>{
                ...
                return (
                  <div key={i} className="dayCell">
                    {isBarStart && (
                      <div className={`bar ${isHot?'hot':'warm'}`} style={{left:0, right:0, width:`calc(${r.duration}00% / ${spanDays})`, transform:`translateX(0)`, gridColumn:`span ${r.duration}`}}>
