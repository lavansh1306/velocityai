...
const open = (tasks ?? []).filter(t=>t.status==='open')
  // date range covering tasks; show at least 5 weeks (1 month+)
...
const minWeeks = 5
const spanWeeks = Math.max(minWeeks, Math.ceil(spanDays/7))
...
// bar refs for arrow anchoring
const barRefs = useRef<Record<string, HTMLDivElement|null>>({})
const setBarRef = (id:string) => (el:HTMLDivElement|null) => { barRefs.current[id] = el }

// compute elbow arrows using real DOM positions (no diagonals)
...
<span className="dot green" style={{marginLeft:16}}></span> On track (> 1 week)
    ...
    {/* Arrows (after measured to avoid hydration mismatches) */}
  {gridW !== null && (
